...
# ============================================================
# Shared: write a static attach-helper script (parent shell, before srun)
# Takes the JOBID as $1 (or $SLURM_JOB_ID env). User reads JOBID from the
# in-container print at startup and runs:  bash $ATTACH_HELPER <jobid>
# ============================================================
ATTACH_HELPER="/home/phuonguyen/container-configs/.last_launch_attach.sh"
CONTAINER_NAME="${IMAGE}-${ARCH}-ct"

cat > "${ATTACH_HELPER}" <<EOF
#!/bin/bash
# Auto-generated by launch.sh — attach another shell to the running container.
# Run from a host shell (cluster login or allocation head), NOT inside the container.
#
# After attaching, test LCP em (HT, fast-build, expert-major, 1n / 8 GPU / topk=2 / experts=32):
#   cd /lustre/fsw/coreai_dlfw_dev/phuonguyen/EP && \\
#     bash ep/scripts/run_nccl_ep_interactive.sh \\
#       --algo ht --fast-build --expert-major --validate --warp-timing \\
#       --gpus 8 --top-k 2 --experts-per-rank 4
JOBID="\${1:-\${SLURM_JOB_ID:-}}"
[ -z "\$JOBID" ] && { echo "Usage: \$0 <jobid>  (or set SLURM_JOB_ID)"; exit 1; }

exec srun --jobid="\$JOBID" --overlap --pty \\
    --container-name="${CONTAINER_NAME}" \\
    --container-workdir="${WORKDIR}" \\
    --container-writable \\
    bash -c 'export HOME=/home/phuonguyen; export PATH=${CLAUDE_BASE}:/home/phuonguyen/.local/bin:\$PATH; exec bash -i'
EOF
chmod +x "${ATTACH_HELPER}"
...
SHARED_INIT+=' && echo " Container ready. JOBID=$SLURM_JOB_ID  NAME='"${CONTAINER_NAME}"'" '
SHARED_INIT+=' && echo " To start a new agent in THIS container from another shell:" '
SHARED_INIT+=' && echo "   bash '"${ATTACH_HELPER}"' \$SLURM_JOB_ID" '
