...
function open() {
  if [ -n "$(which xdg-open 2>/dev/null)" ]; then
    if [ -z "$1" -o "$1" == "-h" -o "$1" == "--help"  ];then
      ...
      echo ""
      echo "To set defaults on *nix use:"
      echo "xdg-mime --help"
      echo "e.g. 'xdg-mime default nautilus.desktop inode/directory'"
      echo "For making 'open .' work in with nautilus."
      echo "For checking default 'xdg-mime query filetype <file>'"
      echo "<app>.desktop files are located in /usr/share/applications"
      echo " and custom ones in .local/share/applications\n"
